...
// Fetch Data KHUSUS LULUS (Alumni) with Violation Stats
$sql = "SELECT m.*, 
...
<body class="text-slate-800 font-sans antialiased h-screen overflow-hidden flex dark:text-slate-100">
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8">
            ...
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-end gap-4 mb-2">
                    <div>
                        <h1 class="text-3xl font-bold tracking-tight text-amber-900 dark:text-white mb-1">Direktori Alumni</h1>
                        <p class="text-slate-500 dark:text-slate-400 text-sm">Menampilkan <?php echo count($students); ?> santri yang telah Lulus.</p>
                    </div>
                </div>
                 ...
                 <div class="glass-panel rounded-2xl p-4 shadow-glass mb-6 sticky top-0 z-30">
                    <div class="flex w-full items-center">
                        <div class="relative w-full">
                            <input type="text" id="searchInput" onkeyup="filterCards()" placeholder="Cari Nama, NIM, Angkatan..." class="w-full pl-10 pr-4 py-3 bg-white/50 dark:bg-slate-700/50 border border-transparent hover:bg-white hover:border-amber-200 hover:shadow-sm focus:bg-white focus:ring-2 focus:ring-accent/20 focus:border-accent rounded-xl outline-none transition-all">
                            <i class="fa-solid fa-search absolute left-3 top-3.5 text-amber-400"></i>
                         </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20">
                        <?php foreach($students as $s): 
                            ...
                            if ($s['violation_count'] == 0) {
                                ...
                                $badgeText = 'Pernah Melanggar';
                                $badgeClass = 'bg-slate-100 text-slate-600 border-slate-200';
                            }
                        ?>
                        <!-- Card Item -->
                        <div class="glass-panel rounded-2xl shadow-glass hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden group h-full relative border border-white/60 searchable-item"
                             data-search="<?php echo strtolower($s['nama'] . ' ' . $s['nim'] . ' ' . $s['asal']); ?>">
                            
                            <!-- Top Banner / Photo -->
                            <div class="h-28 bg-gradient-to-tr from-amber-500 to-yellow-400 relative">
                                <div class="absolute top-0 left-0 w-full h-full bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20"></div>
                                 <div class="absolute top-3 right-3 flex gap-1">
                                    <span class="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider backdrop-blur-md border shadow-sm <?php echo $badgeClass; ?>">
                                        <?php echo $badgeText; ?>
                                    </span>
                                 </div>
                            </div>
                            ...
                            <div class="px-5 -mt-12 flex justify-between items-end relative z-10">
                                <div class="w-24 h-24 rounded-2xl border-4 border-white/80 shadow-lg overflow-hidden bg-white dark:border-slate-800 dark:bg-slate-700">
                                    <?php if($photo): ?>
                                    ...
                                    <?php else: ?>
                                        <div class="w-full h-full flex items-center justify-center bg-amber-50 text-amber-200 dark:bg-slate-600 dark:text-slate-400">
                                ...
                                <div class="flex gap-2 mb-2">
                                    ...
                                    <button onclick='editStudent(<?php echo json_encode($s, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>)' class="w-9 h-9 flex items-center justify-center rounded-xl bg-white/70 hover:bg-white text-amber-500 shadow-sm border border-slate-100 transition-all active:scale-95 tooltip" title="Edit Data">
                                     ...
                                     <button onclick="openHistoryModal(<?php echo $s['id']; ?>)" class="w-9 h-9 flex items-center justify-center rounded-xl bg-white/70 hover:bg-white text-red-500 shadow-sm border border-slate-100 transition-all active:scale-95 tooltip" title="Riwayat Pelanggaran">
                            ...
                            <div class="p-5 flex-1 flex flex-col gap-3">
                                <div>
                                    <h3 class="font-bold text-lg text-slate-800 leading-tight mb-1 truncate dark:text-white" title="<?php echo $s['nama']; ?>"><?php echo formatNameBin($s['nama'], $s['gender'], $s['nama_ayah']); ?></h3>
                                    <div class="text-[10px] text-slate-500 mt-1">
                                        Nomor Induk Mahasantri: <span class="font-mono font-bold text-accent"><?php echo $s['nim']; ?></span>
                                    </div>
                                </div>

                                <hr class="border-amber-100/50 dark:border-slate-700">
                                ...
                                <div class="space-y-2 text-xs text-slate-600 dark:text-slate-300">
                                    ...
                                    <!-- Angkatan -->
                                    <!-- Graduation Year -->
                                    <div class="flex justify-between items-center bg-amber-50/50 dark:bg-slate-700/50 p-2 rounded-lg">
                                        <div class="flex items-center gap-2">
                                            <i class="fa-solid fa-graduation-cap text-amber-500"></i>
                                            <div class="flex flex-col leading-none">
                                                <span class="font-bold text-slate-700 dark:text-white text-[10px]">Lulusan TA</span>
                                                <span class="text-[10px] text-slate-500"><?php echo $s['tahun_lulus_hijriah'] ? $s['tahun_lulus_hijriah'] : '-'; ?></span>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <span class="font-bold text-emerald-600 dark:text-emerald-400"><?php echo $s['tahun_lulus_masehi'] ? $s['tahun_lulus_masehi'] : 'Unknown'; ?></span>
                                        </div>
                                    </div>

                                    <!-- TTL -->
                                    <div class="flex gap-2 items-start" title="Tempat, Tanggal Lahir">
                                        <i class="fa-solid fa-cake-candles w-4 text-center text-pink-500 mt-0.5"></i>
                                        <span class="truncate leading-tight">
                                            <?php echo $s['tempat_lahir'] . ', ' . formatDateId($s['tanggal_lahir']); ?>
                                        </span>
                                    </div>
                ...
                <?php else: ?>
                    <div class="flex flex-col items-center justify-center h-96 text-slate-400">
                        <div class="bg-white/50 p-6 rounded-full shadow-sm mb-4 backdrop-blur-sm">
                            <i class="fa-solid fa-graduation-cap text-6xl text-amber-200"></i>
                        </div>
                        <p class="font-medium text-lg">Belum ada data Alumni Lulus.</p>
                    </div>
                <?php endif; ?>
            </div>
    ...
    <!-- External Scripts (SweetAlert & Custom CRUD) -->
