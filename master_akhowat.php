...
// Fetch Data KHUSUS AKHOWAT with Violation Stats
$sql = "SELECT m.*, 
...
WHERE (m.gender LIKE 'Akhowat%' OR m.gender = 'Perempuan' OR m.gender = 'P') AND m.status = 'Aktif'
GROUP BY m.id
ORDER BY m.nim ASC";
try {
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8">
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-end gap-4">
                    <div>
                        <h1 class="text-3xl font-bold font-heading text-pink-800 dark:text-pink-400">Direktori Akhowat</h1>
                        <p class="text-slate-500 dark:text-slate-400 mt-1">Data lengkap mahasantri Akhowat aktif tahun ajaran ini.</p>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="window.location.reload()" class="w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-500 hover:text-pink-600 hover:border-pink-200 shadow-sm flex items-center justify-center transition-all dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400">
                ...
                <div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl p-5 rounded-2xl border border-white/50 dark:border-slate-700 shadow-lg shadow-pink-500/5 sticky top-0 z-20">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <!-- Search Box -->
                        <div class="md:col-span-3 relative">
                            <input type="text" id="searchInput" placeholder="Cari Nama, NIM, Asal, atau Walisantri..." 
                                class="w-full pl-12 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 outline-none transition-all placeholder:text-slate-400">
                            <i class="fa-solid fa-search absolute left-4 top-3.5 text-slate-400 pointer-events-none"></i>
                        </div>

                        <!-- Filter Mustawa -->
                        <div class="relative">
                            <select id="filterMustawa" class="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-pink-500/20 focus:border-pink-500 outline-none transition-all appearance-none cursor-pointer text-slate-600 dark:text-slate-300">
                    ...
                    <div class="mt-3 flex justify-between items-center text-xs px-1">
                        ...
                        <button onclick="resetFilters()" class="text-pink-600 hover:text-pink-700 font-medium hover:underline hidden" id="resetBtn">Reset Filter</button>
                    </div>
                </div>
                ...
                <div id="studentGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6 pb-20">
                            ...
                            <div class="flex flex-col md:flex-row h-full">
                                ...
                                <div class="w-full md:w-32 bg-slate-50 dark:bg-slate-700/30 flex flex-col items-center justify-center p-4 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-700 relative overflow-hidden">
                                    ...
                                    <div class="mt-2 text-center">
                                         <span class="block text-xs font-bold text-slate-400 font-mono tracking-tighter"><?php echo $s['nim']; ?></span>
                                          <span class="inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase mt-1 bg-pink-50 text-pink-600">
                                            <?php echo $s['gender']; ?>
                                        </span>
                                    </div>
                                    <!-- Background Pattern -->
                                ...
                                <div class="p-4 flex-1 flex flex-col justify-between">
                                    <div>
                                        <!-- Header Info -->
                                        <div class="mb-3">
                                            <h3 class="font-bold text-lg text-slate-800 dark:text-white leading-tight font-heading group-hover:text-pink-600 transition-colors">
                                                <?php echo $s['nama']; ?>
                                            </h3>
                                        ...
                                        <div class="flex flex-wrap gap-2 mb-3">
                                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-pink-50 dark:bg-pink-900/20 text-pink-700 dark:text-pink-300 text-[11px] font-semibold border border-pink-100 dark:border-pink-800">
                    ...
                    <?php else: ?>
                        <div class="col-span-full py-12 text-center text-slate-400">
                            ...
                            <p>Belum ada data Akhowat aktif.</p>
                        </div>
                    <?php endif; ?>
                    
                    <!-- Empty State for Filter -->
                    <div id="noResults" class="hidden col-span-full py-20 text-center">
                        <div class="w-24 h-24 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4 text-pink-500 text-4xl shadow-inner">
