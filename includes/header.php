...
?>
<header class="h-16 bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-100 flex items-center justify-between px-3 md:px-6 z-20 no-print dark:bg-slate-900/80 dark:border-slate-800 transition-colors duration-300">
    <div class="flex items-center gap-2 md:gap-4">
        <button id="mobile-menu-btn" class="hidden text-gray-500 hover:text-blue-600 focus:outline-none md:hidden dark:text-gray-400 dark:hover:text-white transition-colors p-1">
        ...
        <div class="flex flex-col pl-1 md:pl-0">
            <h2 class="text-lg md:text-xl font-bold text-slate-800 font-sans tracking-tight dark:text-white leading-tight">
                <span class="md:hidden">DB Mahasantri</span>
                <span class="hidden md:inline">Database Pusat Mahasantri <span class="font-light text-slate-500 dark:text-slate-400">Jami'ah ABAT Lampung</span></span>
            </h2>
            <p class="hidden md:block text-[10px] uppercase font-bold text-emerald-600 dark:text-emerald-400 tracking-wider mt-0.5">
    ...
    <div class="flex items-center gap-2 md:gap-4">
        ...
        <form action="master_data" method="GET" class="relative hidden md:block group">
            <input 
                type="text" 
                name="q"
        ...
        <div class="flex items-center gap-3 pl-2 border-l border-slate-200 dark:border-slate-700">
            ...
            <div class="relative">
                <div class="h-8 w-8 md:h-10 md:w-10 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 flex items-center justify-center text-blue-700 font-bold border-2 border-white shadow-sm cursor-pointer hover:shadow-md transition-all dark:from-slate-700 dark:to-slate-800 dark:text-slate-300 dark:border-slate-600 text-xs md:text-base" onclick="document.getElementById('logout-menu').classList.toggle('hidden')">
                    <?php 
                ...
                <div id="logout-menu" class="hidden absolute top-12 right-0 bg-white/90 backdrop-blur-xl shadow-xl rounded-xl w-48 py-2 border border-slate-100 z-50 transform origin-top-right transition-all dark:bg-slate-900/90 dark:border-slate-700">
                    ...
                    <a href="profile" class="block px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-blue-600 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-blue-400 transition-colors">
