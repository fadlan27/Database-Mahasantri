...
// Fetch Data KHUSUS AKTIF
// Fetch ALL active students to allow client-side filtering
// Fetch Data KHUSUS AKTIF with Violation Stats
$sql = "SELECT m.*, 
        ...
        <main class="flex-1 overflow-y-auto p-4 md:p-8">
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-end gap-4">
                    <div>
                        <h1 class="text-3xl font-bold font-heading text-emerald-800 dark:text-emerald-400">Direktori Mahasantri</h1>
                        <p class="text-slate-500 dark:text-slate-400 mt-1">Data lengkap mahasantri aktif tahun ajaran ini.</p>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="window.location.reload()" class="w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-500 hover:text-emerald-600 hover:border-emerald-200 shadow-sm flex items-center justify-center transition-all dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400">
                ...
                <div class="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl p-5 rounded-2xl border border-white/50 dark:border-slate-700 shadow-lg shadow-emerald-500/5 sticky top-0 z-20">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <!-- Search Box -->
                        <div class="md:col-span-2 relative">
                            <input type="text" id="searchInput" placeholder="Cari Nama, NIM, Asal, atau Walisantri..." 
                                class="w-full pl-12 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all placeholder:text-slate-400">
                            <i class="fa-solid fa-search absolute left-4 top-3.5 text-slate-400 pointer-events-none"></i>
                        </div>

                        <!-- Filter Mustawa -->
                        <div class="relative">
                            <select id="filterMustawa" class="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all appearance-none cursor-pointer text-slate-600 dark:text-slate-300">
                        ...
                        <!-- Filter Gender -->
                        <div class="relative">
                            <select id="filterGender" class="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all appearance-none cursor-pointer text-slate-600 dark:text-slate-300">
                                <option value="">Semua Gender</option>
                                <option value="Ikhwan">Ikhwan</option>
                                <option value="Akhowat">Akhowat</option>
                            </select>
                            <i class="fa-solid fa-venus-mars absolute left-4 top-3.5 text-slate-400 pointer-events-none"></i>
                            <i class="fa-solid fa-chevron-down absolute right-4 top-4 text-xs text-slate-400 pointer-events-none"></i>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 flex justify-between items-center text-xs px-1">
                        ...
                        <button onclick="resetFilters()" class="text-emerald-600 hover:text-emerald-700 font-medium hover:underline hidden" id="resetBtn">Reset Filter</button>
                    </div>
                </div>
                ...
                <div id="studentGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6 pb-20">
                    <?php if(count($students) > 0): ?>
                        ...
                        <div onclick='viewStudent(<?php echo json_encode($s, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP); ?>)' class="student-card group bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden relative flex flex-col cursor-pointer"
                             ...
                             data-asal="<?php echo strtolower($s['asal_ppui'] ?? ''); ?>"
                             data-city="<?php echo strtolower($s['kabupaten'] ?? ''); ?>"
                            ...
                            <div class="flex flex-col md:flex-row h-full">
                                <!-- Left: Photo contents... -->
                                <!-- Code shortened for replacement block clarity, assuming inner content unchanged unless specified -->
                                <div class="w-full md:w-32 bg-slate-50 dark:bg-slate-700/30 flex flex-col items-center justify-center p-4 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-700 relative overflow-hidden">
                                    ...
                                    <div class="mt-2 text-center">
                                         <span class="block text-xs font-bold text-slate-400 font-mono tracking-tighter"><?php echo $s['nim']; ?></span>
                                          <span class="inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase mt-1 <?php echo $s['gender'] == 'Ikhwan' ? 'bg-blue-50 text-blue-600' : 'bg-pink-50 text-pink-600'; ?>">
                                            <?php echo $s['gender']; ?>
                                        </span>
                                    </div>
                                    <!-- Background Pattern -->
                                ...
                                <div class="p-4 flex-1 flex flex-col justify-between">
                                    <div>
                                        <!-- Header Info -->
                                        <div class="mb-3">
                                            <h3 class="font-bold text-lg text-slate-800 dark:text-white leading-tight font-heading group-hover:text-emerald-600 transition-colors">
                                                <?php echo $s['nama']; ?>
                                            </h3>
                                        ...
                                        <div class="flex flex-wrap gap-2 mb-3">
                                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 text-[11px] font-semibold border border-emerald-100 dark:border-emerald-800">
                    ...
                    <?php else: ?>
                        <div class="col-span-full py-12 text-center text-slate-400">
                            ...
                            <p>Belum ada data mahasantri aktif.</p>
                        </div>
                    <?php endif; ?>
                    
                    <!-- Empty State for Filter -->
                    <div id="noResults" class="hidden col-span-full py-20 text-center">
                        <div class="w-24 h-24 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4 text-emerald-500 text-4xl shadow-inner">
    ...
    <script>
        ...
        function filterStudents() {
            ...
            cards.forEach(card => {
                ...
                const asal = card.dataset.asal; // This is PPUI now
                const city = card.dataset.city; // New: Kabupaten
                const wali = card.dataset.wali;


                // Logic matches
                const matchesSearch = name.includes(query) || nim.includes(query) || asal.includes(query) || city.includes(query) || wali.includes(query);
                const matchesMustawa = mustawa === '' || cardMustawa === mustawa;
                const matchesGender = gender === '' || cardGender === gender;

                if(matchesSearch && matchesMustawa && matchesGender) {
        ...
        filterGender.addEventListener('change', filterStudents);
        ...
        // Auto-filter from URL Param (e.g. ?search=PPUI)
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if(searchParam) {
                ...
                filterStudents(); // Trigger filter immediately
            }
        });
    </script>
